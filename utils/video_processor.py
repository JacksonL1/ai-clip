...
class VideoProcessor:
    ...
    def create_collection(clips_list: List[Path], output_path: Path) -> bool:
        ...
        try:
            ...
            if result.returncode == 0:
                print(f"成功创建合集: {output_path}")
                return True
            else:
                print(f"创建合集失败: {result.stderr}")
                print(f"FFmpeg stdout: {result.stdout}")
                return False

        except Exception as e:
            print(f"视频拼接异常: {str(e)}")
            return False
    ...
    def extract_thumbnail(video_path: Path, output_path: Path, time_offset: int = 5) -> bool:
        ...
        try:
            ...
            if result.returncode == 0 and output_path.exists():
                ...
                print(f"提取缩略图失败: {result.stderr}")
                return False

        except Exception as e:
            print(f"提取缩略图异常: {str(e)}")
            return False
        ...
        for collection_data in collections_data:
            ...
            for clip_id in clip_ids:
                ...
                if found_clips:
                    ...
                    print(f"找到合集 {collection_id} 的切片: {found_clip.name}")
                else:
            ...
            if clips_list:
                ...
                if VideoProcessor.create_collection(clips_list, output_path):
                    ...
                    print(f"成功创建合集 {collection_id}: {output_path}")
            else:
